...
describe('clone', () => {
  ...
  it('If proto option is true, copy proto properties', () => {
    const input = Object.create({ a: 1 });
    const actual = clone(input, true);
  ...
  it('Can clone circular objects with proto', () => {
    const input = { nest: { a: 1, b: 2, c: Object.create({ a: 1 }) } };
    input.nest.circular = input;
    const actual = clone(input, true);
    expect(actual.nest.a).toEqual(1);
    expect(actual.nest.b).toEqual(2);
    expect(actual.nest.c).toEqual({ a: 1 });
    expect(actual.nest.circular).toBe(actual);
  });
  it('Can clone buffers from object', () => {
    ...
    const actual = clone(input);
    expect(actual.a).not.toBe(buffer);
  ...
  it('Can clone TypedArrays from object', () => {
    ...
    expect(actual.a).not.toBe(input.a);
  ...
  it('Can clone buffers from object - proto', () => {
    const buffer = Buffer.from('123456789abcdefghi');
    const input = { a: buffer };
    const actual = clone(input, true);
    expect(actual.a).not.toBe(buffer);
  ...
  it('Can clone numbers - proto', () => {
    const input = 42;
    const actual = clone(input, true);
    expect(actual).toEqual(input);
  });
  it('Can clone strings - proto', () => {
    const input = '42';
    const actual = clone(input, true);
    expect(actual).toEqual(input);
  });
  it('Can clone booleans - proto', () => {
    const input = true;
    const actual = clone(input, true);
    expect(actual).toEqual(input);
  });
  it('Can clone functions - proto', () => {
    const input = () => {};
    const actual = clone(input, true);
    expect(actual).toBe(input);
  });
  it('Can clone dates - proto', () => {
    const input = new Date();
    const actual = clone(input, true);
    expect(actual).toEqual(input);
    expect(actual).not.toBe(input);
  });
  it('Can clone maps - proto', () => {
    const input = new Map([['a', 1]]);
    const actual = clone(input, true);
    expect(actual).toEqual(input);
    expect(actual).not.toBe(input);
  });
  it('Can clone sets - proto', () => {
    const input = new Set(['a']);
    const actual = clone(input, true);
    expect(actual).toEqual(input);
    expect(actual).not.toBe(input);
