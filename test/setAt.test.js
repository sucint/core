...
describe('setAt', () => {
  ...
  it('Should throw an exception if input is undefined or null', () => {
    expect(() => setAt(undefined, 0, 5)).toThrowError(
      'Cannot perform setAt on undefined'
    );
    expect(() => setAt(null, 0, 5)).toThrowError(
      'Cannot perform setAt on null'
    );
  });
  it('Should throw an exception if input is not valid for setAt', () => {
    expect(() => setAt(5, 0, 5)).toThrowError('Cannot perform setAt on input');
  });
